...
export const QuizPlayer = ({
  ...
  if (!questions || questions.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        <CardRounded>
          <h2 style={{ fontSize: '1.4rem', color: 'var(--text-main)', marginBottom: '10px' }}>No Questions Found</h2>
          <p style={{ color: 'var(--text-muted)', marginBottom: '20px' }}>No questions available for this topic yet.</p>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '10px' }}>
        ...
        <CardRounded style={{ textAlign: 'center', padding: '30px 20px', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '25px', flexWrap: 'wrap' }}>
            <div style={{ background: 'var(--bg-main)', border: '2px solid var(--secondary, #bae6fd)', borderRadius: '16px', padding: '14px 24px', minWidth: '120px' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--secondary, #0284c7)', fontWeight: '700', textTransform: 'uppercase' }}>Score</div>
              <div style={{ fontSize: '1.8rem', fontWeight: '900', color: 'var(--text-main)' }}>{correctTotal} / {totalQ}</div>
            </div>
            <div style={{ background: 'var(--bg-main)', border: '2px solid rgba(34, 197, 94, 0.4)', borderRadius: '16px', padding: '14px 24px', minWidth: '120px' }}>
              <div style={{ fontSize: '0.85rem', color: '#16a34a', fontWeight: '700', textTransform: 'uppercase' }}>Accuracy</div>
              <div style={{ fontSize: '1.8rem', fontWeight: '900', color: '#16a34a' }}>{scorePct}%</div>
            </div>
            <div style={{ background: 'var(--bg-main)', border: '2px solid rgba(234, 179, 8, 0.4)', borderRadius: '16px', padding: '14px 24px', minWidth: '120px' }}>
              <div style={{ fontSize: '0.85rem', color: '#ca8a04', fontWeight: '700', textTransform: 'uppercase' }}>XP Earned</div>
              <div style={{ fontSize: '1.8rem', fontWeight: '900', color: '#eab308' }}>+{correctTotal * xpRewardPerQ}</div>
            </div>
          </div>
          ...
          {userAnswersHistory.map((item, idx) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '8px' }}>
                <div style={{ fontWeight: '700', color: 'var(--text-main)', fontSize: '1rem', lineHeight: '1.4' }}>
                  <span style={{ color: 'var(--text-muted)', marginRight: '6px' }}>Q{item.questionIndex}.</span>
                  {item.questionText}
                </div>
                <div>
                  {item.isCorrect ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(34, 197, 94, 0.15)', color: '#22c55e', fontWeight: '700', fontSize: '0.8rem', padding: '4px 10px', borderRadius: '12px', whiteSpace: 'nowrap' }}>
                      <CheckCircle2 size={14} /> Correct
                    </span>
                  ) : (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(239, 68, 68, 0.15)', color: '#ef4444', fontWeight: '700', fontSize: '0.8rem', padding: '4px 10px', borderRadius: '12px', whiteSpace: 'nowrap' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', fontSize: '0.9rem', marginBottom: '8px' }}>
                <div>
                  ...
                  <span style={{ fontWeight: '700', color: item.isCorrect ? '#22c55e' : '#ef4444' }}>
                ...
                {!item.isCorrect && (
                  <div>
                    ...
                    <span style={{ fontWeight: '700', color: '#22c55e' }}>{item.correctAnswer}</span>
                  </div>
                )}
              </div>

              {item.explanation && (
                <div style={{ fontSize: '0.88rem', color: 'var(--text-main)', background: 'var(--primary-light, rgba(99, 102, 241, 0.08))', padding: '10px 14px', borderRadius: '8px', lineHeight: '1.45', border: '1px solid var(--border-light)' }}>
  ...
  return (
    <div style={{ maxWidth: '750px', margin: '0 auto', padding: '10px' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '6px' }}>
            <span style={{ display: 'inline-block', background: 'var(--primary-light, #e0f2fe)', color: 'var(--primary, #0284c7)', fontWeight: '800', fontSize: '0.78rem', padding: '3px 10px', borderRadius: '8px', textTransform: 'uppercase' }}>
              {classNameText}
            </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span style={{ fontSize: '0.85rem', fontWeight: '800', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
      ...
      {/* Main Question Card */}
      <CardRounded style={{ padding: '28px 24px', marginBottom: '20px', boxShadow: 'var(--shadow-md, 0 8px 24px rgba(0,0,0,0.06))' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ background: 'var(--primary-light, #f1f5f9)', color: 'var(--text-muted, #475569)', fontSize: '0.75rem', fontWeight: '700', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '6px' }}>
              Written Answer
            </span>
            <span style={{
              fontSize: '0.75rem',
              ...
              padding: '4px 10px',
              borderRadius: '6px',
              background: questionDiff.toLowerCase() === 'easy' ? 'rgba(34, 197, 94, 0.15)' : questionDiff.toLowerCase() === 'medium' ? 'rgba(234, 179, 8, 0.15)' : 'rgba(239, 68, 68, 0.15)',
              color: questionDiff.toLowerCase() === 'easy' ? '#22c55e' : questionDiff.toLowerCase() === 'medium' ? '#eab308' : '#ef4444'
            }}>
              {questionDiff}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {formulaClue && (
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary, #6366f1)', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', fontWeight: '700' }}
              >
        ...
        {showFormula && formulaClue && (
          <div style={{ background: 'var(--primary-light, #eef2ff)', border: '1.5px solid var(--border-light, #c7d2fe)', borderRadius: '12px', padding: '12px 16px', marginBottom: '16px', fontSize: '0.9rem', color: 'var(--text-main)' }}>
            <div style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--primary, #4f46e5)', textTransform: 'uppercase', marginBottom: '4px' }}>
              📖 Reference Rule: {formulaClue.title}
            </div>
            <div style={{ fontFamily: 'monospace', fontWeight: '700', fontSize: '1rem', color: 'var(--text-main)' }}>
              {formulaClue.formula}
            </div>
          </div>
        )}
        ...
        {showHint && questionHint && (
          <div style={{ background: 'rgba(234, 179, 8, 0.1)', border: '1px dashed #facc15', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', fontSize: '0.92rem', color: '#eab308', lineHeight: '1.45' }}>
            💡 <strong>Hint:</strong> {questionHint}
          </div>
        )}

        {/* Question Text */}
        <div style={{ fontSize: '1.25rem', fontWeight: '700', color: 'var(--text-main)', lineHeight: '1.55', marginBottom: '24px' }}>
          {questionPrompt}
        </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input
              ...
              style={{
                ...
                background: isAnswered ? (isCorrect ? 'rgba(34, 197, 94, 0.12)' : 'rgba(239, 68, 68, 0.12)') : 'var(--bg-card-solid)',
                color: 'var(--text-main)',
                outline: 'none',
        ...
        {isAnswered && (
          ...
          >
            ...
            {!isCorrect && revealSolution && (
              <div style={{ fontSize: '0.95rem', color: '#ef4444', marginBottom: '8px' }}>
                {t('solution')}: <strong style={{ color: '#22c55e', fontSize: '1.05rem' }}>{questionAnswer}</strong>
              </div>
            )}

            {(isCorrect || revealSolution) && questionExp && (
              <div style={{ fontSize: '0.92rem', color: 'var(--text-main)', background: 'var(--bg-main)', padding: '12px 16px', borderRadius: '10px', marginTop: '8px', lineHeight: '1.5', border: '1px solid var(--border-light)' }}>
